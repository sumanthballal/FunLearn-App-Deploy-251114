# FunLearn_Final.ps1
# One-click setup, repair, and launcher for FunLearn (no emoji, ASCII only)
...
Write-Host "`n=== Starting FunLearn auto-setup in $root ===`n"

if (-not (Test-Path $backend) -or -not (Test-Path $frontend)) {
    Write-Host "ERROR: backend or frontend folder missing." -ForegroundColor Red
    exit 1
}
...
Write-Host "Created emotion_model.py."
    ...
    Write-Host "emotion_model.py already exists."
}
...
Write-Host "Batch launchers created."
...
Write-Host "`nFunLearn repaired and launched successfully."
